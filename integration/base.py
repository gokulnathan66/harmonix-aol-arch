...
class IntegrationResult:
    ...
    @classmethod
    def fail(
        cls, error: str, error_code: str = None, latency_ms: float = 0, **metadata
    ) -> "IntegrationResult":
...
class BaseIntegration(ABC):
    ...
    async def execute(
        self, action: str, payload: Dict[str, Any] = None, **kwargs
    ) -> IntegrationResult:
        """
        ...
        try:
            ...
            return IntegrationResult.ok(
                data=result, latency_ms=latency_ms, action=action
            )
            ...
            return IntegrationResult.fail(
                error=str(e), latency_ms=latency_ms, action=action
            )

    @abstractmethod
    async def _do_execute(self, action: str, payload: Dict[str, Any], **kwargs) -> Any:
