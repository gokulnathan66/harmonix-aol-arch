...
service_counter = Counter(
    "service_operations_total", "Total service operations", ["operation", "status"]
)
service_duration = Histogram(
    "service_operation_duration_seconds", "Operation duration", ["operation"]
)
active_requests = Gauge("service_active_requests", "Currently active requests")
event_counter = Counter(
    "service_events_total", "Events published/received", ["direction", "topic"]
)
...
class AOLService:
    ...
    def __init__(
        self, config_path: str = "config.yaml", manifest_path: str = "manifest.yaml"
    ):
        # Load configuration
        ...
        self.logger = setup_logging(
            {
                "spec": {
                    "logging": {
                        "level": self.config.get("monitoring", {}).get(
                            "logLevel", "INFO"
                        ),
                        "format": "json",
                    }
        ...
        setup_tracing(
            {
                ...
                "spec": {
                    "monitoring": {
                        "tracingEnabled": self.config.get("monitoring", {}).get(
        ...
        consul_port = int(
            os.getenv("CONSUL_HTTP_ADDR", "consul-server:8500").split(":")[1]
            if ":" in os.getenv("CONSUL_HTTP_ADDR", "consul-server:8500")
            else 8500
        )
        ...
        self.health_reporter = HealthReporter(
            config=self.config, aol_core_endpoint=aol_core_endpoint
        )
    ...
    def _get_service_name(self) -> str:
        ...
        return (
            self.manifest.get("metadata", {}).get("name")
            or self.config.get("service", {}).get("name")
            or self.config.get("metadata", {}).get("name")
            or self.config.get("name", "aol-service")
        )
    ...
    def _validate_configuration(self):
        ...
        if os.path.exists(manifest_path):
            ...
            if not result.valid:
                self.logger.warning(
                    f"Manifest validation warnings: {len(result.warnings)} warnings, {len(result.errors)} errors"
                )
    ...
    def _register_lifecycle_hooks(self):
        ...
        self.health_reporter.register_startup_hook(
            name="initialize_connections", hook_fn=self._on_startup
        )

        # Ready hooks
        self.health_reporter.register_ready_hook(
            name="subscribe_to_events", hook_fn=self._on_ready
        )

        # Shutdown hooks
        self.health_reporter.register_shutdown_hook(
            name="cleanup_connections", hook_fn=self._on_shutdown
        )

        # Pre-stop hooks
        self.health_reporter.register_pre_stop_hook(
            name="drain_requests", hook_fn=self._on_pre_stop
        )

        # Register health checks
        self.health_reporter.register_health_check(
            name="data_client", check_fn=self._check_data_client_health, critical=False
        )

        self.health_reporter.register_health_check(
            name="consul", check_fn=self._check_consul_health, critical=True
        )
    ...
    async def _on_ready(self):
        ...
        if self.event_bus:
            ...
            await self.event_bus.subscribe(
                topic=f"task.{self.service_name}", handler=self._handle_task_request
            )
        ...
        await self._publish_event(
            ...
            payload={"service": self.service_name, "kind": self.service_kind},
        )
    ...
    async def _on_pre_stop(self):
        ...
        grace_period = (
            self.config.get("health", {}).get("lifecycle", {}).get("preStopDelay", 5)
        )
    ...
    def _register_self(self):
        ...
        grpc_port = int(
            endpoints.get("grpc")
            or self.config.get("spec", {}).get("grpcPort")
            or 50050
        )
        health_port = int(
            endpoints.get("health")
            or self.config.get("spec", {}).get("healthPort")
            or 50200
        )
        metrics_port = int(
            endpoints.get("metrics")
            or self.config.get("spec", {}).get("metricsPort")
            or 8080
        )
        ...
        try:
            self.consul_client.agent.service.register(
                ...
                check=consul.Check.http(
                    url=f"http://{hostname}:{health_port}/health", interval="10s"
                ),
            )
    ...
    def _initialize_data_client(self):
        """Initialize data client if enabled"""
        data_config = self.config.get("dataClient") or self.config.get("spec", {}).get(
            "dataClient", {}
        )
        ...
        if not data_config.get("enabled", False) and not data_reqs.get(
            "enabled", False
        ):
        ...
        self.data_client = DatabaseClient(
            aol_core_endpoint=aol_core_endpoint, service_name=self.service_name
        )
    ...
    async def _initialize_collections(self):
        ...
        for collection_spec in collections:
            ...
            try:
                ...
                self.logger.info(
                    f"✓ Collection '{collection_name}' ready: {collection_id}"
                )
                initialized_count += 1
            except Exception as e:
                self.logger.error(
        ...
        self.logger.info(
            f"Collection initialization: {initialized_count}/{len(collections)} successful"
        )
    ...
    async def Process(self, request: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            with service_duration.labels(operation="process").time():
                ...
                if self.data_client:
                    try:
                        doc_id = await self.data_client.insert(
                            "events",
                            {
                                "request_id": request_id,
                                "service": self.service_name,
...
class AOLServiceApp:
    ...
    def __init__(self):
        ...
        self.app.router.add_post(
            "/api/execute", self.process_handler
        )  # Alias for tools
    ...
    def signal_handler(sig, frame):
        async def shutdown():
            ...
            try:
                ...
                app.service.consul_client.agent.service.deregister(
