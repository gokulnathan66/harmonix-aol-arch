...
class LoadBalancedGRPCClient:
    ...
    async def _get_service_instances(self) -> List[Dict]:
        ...
        # Query aol-core
        instances = await self.discovery_client.discover_service(self.service_name, healthy_only=True)
        ...
        if self.circuit_breaker.state == "OPEN":
            if self.circuit_breaker.last_failure_time:
                ...
                if elapsed > self.circuit_breaker.timeout:
                    ...
                    self.logger.info(f"Circuit breaker HALF_OPEN for {self.service_name}")
                else:
