...
class DatabaseClient:
    ...
    async def _get_db_endpoint(self) -> str:
        ...
        instances = await self.discovery_client.discover_service("knowledge-db", healthy_only=True)
        ...
        self._db_endpoint = f"http://{instance['address']}:{instance.get('health_port', 8084)}"
        return self._db_endpoint
        ...
        try:
            async with session.post(
                ...
                if resp.status == 200:
                    ...
                    raise PermissionDeniedError(f"No permission to access '{full_name}'")
                else:
                    error = await resp.text()
