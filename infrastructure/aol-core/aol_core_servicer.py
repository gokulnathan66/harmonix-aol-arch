...
class AOLCoreServicer:
    ...
    async def DeregisterService(self, request, context):
        ...
        try:
            await self.registry.deregister_service(request.service_name, request.service_id)
            ...
            for service_name, instances in services.items():
                for instance in instances:
                    if request.filter_type:
                        service_type = instance.manifest.get('metadata', {}).get('labels', {}).get('aol.service.type')
