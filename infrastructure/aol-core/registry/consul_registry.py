...
class ConsulServiceRegistry:
    ...
    def __init__(self, consul_host=None, consul_port=8500):
        ...
        host = consul_host or os.getenv('CONSUL_HTTP_ADDR', 'consul-server').split(':')[0]
        port = consul_port
    ...
    def register_service(self, instance: ServiceInstance) -> bool:
        ...
        try:
            ...
            if success:
                self.logger.info(f"Registered service: {instance.name} ({instance.id}) at {instance.address}:{instance.port}")
            return success
    ...
    def discover_service(self, service_name: str, healthy_only=True) -> List[ServiceInstance]:
