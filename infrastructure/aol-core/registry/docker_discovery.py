...
class DockerDiscovery:
    ...
    def __init__(self, config, registry):
        ...
        try:
            docker_socket = config.get('spec', {}).get('discovery', {}).get('dockerSocket', '/var/run/docker.sock')
            ...
            try:
                ...
                self.logger.info(f"Docker discovery initialized successfully using socket: {docker_socket}")
            except Exception as e1:
                # Method 2: Try without version (let it auto-detect)
                try:
                    self.docker_client = docker.DockerClient(base_url=base_url)
                    self.docker_client.ping()
                    self.logger.info(f"Docker discovery initialized using socket (no version): {docker_socket}")
                    ...
                    try:
                        ...
                        self.docker_client = docker.DockerClient(base_url=base_url)
                        self.docker_client.ping()
                        self.logger.info(f"Docker discovery initialized via APIClient: {docker_socket}")
                    except Exception as e3:
                        # All methods failed
                        raise e1 from e3
        except Exception as e:
            self.logger.warning(f"Failed to connect to Docker: {e}. Docker discovery will be disabled.")
            self.logger.debug(f"Docker connection error details: {type(e).__name__}: {str(e)}", exc_info=True)
            self.docker_client = None
    
    async def watch_containers(self):
        """Watch for container changes"""
        self.running = True
        label_prefix = self.config.get('spec', {}).get('discovery', {}).get('labelPrefix', 'aol.service')
        refresh_interval = self.config.get('spec', {}).get('discovery', {}).get('refreshInterval', '10s')
        ...
        try:
            ...
            for container in containers:
                ...
                if f'{label_prefix}' in labels and labels[f'{label_prefix}'] == 'true':
                    ...
                    if service_name:
                        await self._register_from_container(container, service_name, labels)
    ...
    async def _discover_via_cli(self, label_prefix: str):
        ...
        try:
            ...
            docker_socket = self.config.get('spec', {}).get('discovery', {}).get('dockerSocket', '/var/run/docker.sock')
            
            # Get list of containers using Docker HTTP API
            containers = await self._docker_api_request('GET', '/containers/json?all=false')
            ...
            for container in containers:
                try:
                    ...
                    container_details = await self._docker_api_request('GET', f'/containers/{container_id}/json')
                    ...
                    if f'{label_prefix}' in labels and labels[f'{label_prefix}'] == 'true':
                        ...
                        if service_name:
                            await self._register_from_labels(service_name, labels, ports, container_name)
                            
                except Exception as e:
                    self.logger.debug(f"Error processing container {container.get('Names', ['unknown'])[0]}: {e}")
                    continue
            ...
            async with aiohttp.ClientSession(connector=connector) as session:
                async with session.request(method, url) as response:
                    if response.status == 200:
                        ...
                        self.logger.debug(f"Docker API request failed: {response.status} for {path}")
                        return None
    ...
    async def _register_from_labels(self, service_name: str, labels: Dict, ports: Dict, container_name: str):
        ...
        try:
            ...
            self.logger.info(f"Registered service via CLI: {service_name} on port {grpc_port}")
            
        except Exception as e:
            self.logger.error(f"Failed to register service {service_name} from labels: {e}")
    
    async def _register_from_container(self, container, service_name: str, labels: Dict):
