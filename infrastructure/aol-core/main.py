...
class AOLCore:
    def __init__(self, config_path='config.yaml'):
        ...
        self.health_manager = HealthManager(self.config, self.registry, self.event_store)
    ...
    async def health_handler(self, request):
        ...
        return web.json_response({
            'status': 'healthy',
            'services': len(await self.registry.list_services())
        })
    ...
    async def start(self):
        ...
        try:
            ...
            self.logger.warning("gRPC services will not be available until protobuf files are generated")
