...
class ProtoRegistry:
    ...
    def __init__(self, proto_storage_path: str = "/tmp/proto-registry"):
        ...
        self.proto_files: Dict[str, Dict[str, str]] = {}  # {service_name: {filename: content}}
    ...
    def list_protos(self, service_name: Optional[str] = None) -> Dict:
        ...
        if service_name:
            if service_name in self.proto_files:
                return {
                    service_name: list(self.proto_files[service_name].keys())
                }
            # Check disk
            service_dir = self.proto_storage_path / service_name
            if service_dir.exists():
                return {
                    service_name: [f.name for f in service_dir.glob("*.proto")]
                }
            return {}
    ...
    async def upload_proto(request):
        ...
        try:
            ...
            if not filename.endswith('.proto'):
                return web.json_response({'error': 'File must have .proto extension'}, status=400)
