...
class ToolExecutor:
    ...
    def register_tool(self, tool_config: ToolConfig):
        ...
        self._circuit_breakers[tool_config.name] = CircuitBreaker(
            ...
            failure_threshold=self.config.get('resilience', {}).get(
                'circuitBreaker', {}
            ).get('failureThreshold', 5),
            timeout=self.config.get('resilience', {}).get(
                'circuitBreaker', {}
            ).get('timeout', 60)
        )
...
class Sidecar:
    ...
    async def start(self):
        ...
        self._running = True
        self.logger.info(f"Starting sidecar for {self.service_name} ({self.service_kind})")
