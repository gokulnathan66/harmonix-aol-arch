...
class HealthReporter:
    ...
    def __init__(
        self,
        config: Dict[str, Any],
        aol_core_endpoint: Optional[str] = None
    ):
        self.config = config
        ...
        self.service_name = (
            ...
            config.get('service', {}).get('name') or
            config.get('name', 'aol-service')
        )
    ...
    def register_startup_hook(
        ...
        self._lifecycle_hooks['startup'].append(LifecycleHook(
            name=name,
            hook_fn=hook_fn,
            timeout=timeout
        ))
    ...
    def register_ready_hook(
        ...
        self._lifecycle_hooks['ready'].append(LifecycleHook(
            name=name,
            hook_fn=hook_fn,
            timeout=timeout
        ))
    ...
    def register_shutdown_hook(
        ...
        self._lifecycle_hooks['shutdown'].append(LifecycleHook(
            name=name,
            hook_fn=hook_fn,
            timeout=timeout
        ))
    ...
    def register_pre_stop_hook(
        ...
        self._lifecycle_hooks['pre_stop'].append(LifecycleHook(
            name=name,
            hook_fn=hook_fn,
            timeout=timeout
        ))
    ...
    async def _send_heartbeat(self):
        ...
        try:
            ...
            async with session.post(
                f"{self.aol_core_endpoint}/api/health/heartbeat",
                json=payload
            ) as resp:
                if resp.status == 200:
                    ...
                    self._failed_heartbeats += 1
                    self.logger.warning(
                        f"Heartbeat rejected: HTTP {resp.status}"
                    )
    ...
    async def report_health(self, status: str):
        ...
        try:
            ...
            self._status = HealthStatus.HEALTHY if status == "healthy" else HealthStatus.UNHEALTHY
